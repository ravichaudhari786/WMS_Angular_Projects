...
export class TransferComponent implements OnInit {
  ...
  OnSaveTransfer(formData: any) {
    ...
    if (this.form.value.cbCustomerID == null || this.form.value.cbCustomerID == "") {
      this.dialog.alert("Please .... select From customers......!!");
      document?.getElementById("cbCustomerID")?.focus();
      return;
    } else
      if (this.form.value.cbToCustomerID == null || this.form.value.cbToCustomerID == "") {
        this.dialog.alert("Please .... select To Customer");
        document?.getElementById("cbToCustomerID")?.focus();
        return;
      } else if (this.form.value.Transfer_date == null || this.form.value.Transfer_date == "") {
        this.dialog.alert("Please .... Enter Transfer date");
        document?.getElementById("Transfer_date")?.focus();
        return;
      } else if (this.form.value.txtOrderGivenBy == null || this.form.value.txtOrderGivenBy == "") {
        this.dialog.alert("Please .... Enter Order Given By");
        document?.getElementById("txtOrderGivenBy")?.focus();
        return;
      } else if (this.TransferDetailsList.length==0) {
        this.dialog.alert("Please... Add Transfer Details..!!!");
        //document?.getElementById("txtOrderGivenBy")?.focus();
        return;
      } else{
        ...
        this.SaveTransferData = {
          ...
          TD_TransferDetail: this.TransferDetailsListSave,
          TD_TransferCharges: this.TransferChargesListSave,
  ...
  OnCustomerarea(ckData: any) {
    if (this.form.value.ckCustomerarea == true) {
      if (this.form.value.cbToCustomerID == null || this.form.value.cbToCustomerID == "") {
        this.dialog.alert("Please... Select To CustomerID ..!!!");
        document?.getElementById("cbToCustomerID")?.focus();
        ...
        this.api.post('/Transfer/GetStorageArea', StorageDataCheck).subscribe(
          data => {
            this.LocationList = data; //console.log(this.LocationList);
          }, error => { console.error(error); }
        );
      }
    } else {
  ...
  onProductListRowClicked(event: any) {
    ...
    this.api.post('/Transfer/GetTransferServices', ChargeData).subscribe(
      data => {
        this.TransferServiceList = data;    //console.log("onProductListRowClicked",this.TransferServiceList);
      }, error => { console.error(error); });
  ...
  OnAddClick() {
    ...
    if (this.form.value.cbproduct_id == null || this.form.value.cbproduct_id == "" || this.form.value.cbproduct_id == "0") {
      document?.getElementById("cbproduct_id")?.focus();
      this.dialog.alert("Please .... Select Product Name...");
      return;
    } else if (this.form.value.txtTransferQty == null || this.form.value.txtTransferQty == "" || this.form.value.txtTransferQty == "0") {
      document?.getElementById("txtTransferQty")?.focus();
  ...
  onRowDblclickedTransfer(Data: any, event: any) {
    ...
    this.api.post('/Transfer/GetTransferServices', ChargeData).subscribe(
      data => {
        this.TransferServiceList = data; //console.log("TransferServiceList",this.TransferServiceList);
