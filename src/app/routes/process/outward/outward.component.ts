...
export class OutwardComponent implements OnInit {
  ...
  OnActionOutward(OutActionata: any) {
    if (String(OutActionata.actions) == 'Delete') {
      if (window.confirm("Do you want to cancel your Outward...!!!")) {
        const OutwardDeleteData = {
          outwardID: OutActionata.rowData.OutWardID,
          warehouseID: OutActionata.rowData.WarehouseID,
          outWardDate: "",
          billStartDate: "",
          deliveryOrderID: 0,
          customerPartyID: 0,
          ...
          transferID: 0,
          dispatchID: 0,
          StatusID: OutActionata.rowData.StatusID,
        };
        this.api.post('/Outward/OutwardStatus_validation', OutwardDeleteData).subscribe(
    ...
    } else if (String(OutActionata.actions) == 'Print') {
      this.api.get('/Outward/OutwardReceipt?OutwardID='+OutActionata.rowData.OutWardID+"&WarehouseID="+this.currentUser.warehouseId).subscribe(
        data => {
          data;
          console.log("OutActionata=>",data)
  ...
  Columnsdata: ColDef[] = [
    {
      headerName: 'Action', width: 200, floatingFilter: false,
      cellRenderer: 'buttonRenderer',
      cellRendererParams: {
        onClick: this.OnActionOutward.bind(this),
        label: 'Click 1'
      }
    },
